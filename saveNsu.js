...
const main = async () => {
  try {
    ...
    for (const transaction of transactions) {
      const nsu = transaction?.response?.nsu ? parseInt(transaction?.response?.nsu) : undefined;

      if (nsu) {
        await prisma.accountTransaction.update({ where: { id: transaction.id }, data: { nsu } });
    ...
    for (const transaction of graphicTransactions) {
      ...
      if (nsu) {
        await prisma.graphicAccountTransaction.update({ where: { id: transaction.id }, data: { nsu } });
